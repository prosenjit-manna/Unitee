...
if ($porto_settings['product-hot']) {
    ...
    if ($featured) {
        $hot_html = '<div class="onhot">'. __('Hot', 'porto') .'</div>';
        $labels .= $hot_html;
    }
}
if ($porto_settings['product-sale']) {
    if ($product->is_on_sale()) {
        ...
        if ($porto_settings['product-sale-percent'] && $percentage)
            ...
            $sales_html = apply_filters('woocommerce_sale_flash', '<div class="onsale">'.__( 'Sale', 'porto' ).'</div>', $post, $product);
        $labels .= $sales_html;
    }
}
...
if ($porto_settings['product-stock']) {
    ...
    if ( $availability['availability'] == __( 'Out of stock', 'woocommerce' )) {
        ...
        echo '<div data-link="' . get_permalink( wc_get_page_id( 'cart' ) ) . '" class="viewcart viewcart-'.$product->id.'" title="' . __('View Cart', 'woocommerce') . '"></div>';
    }
}
