...
class Notifications {
    ...
    public function IsRead($id, $read = TRUE) {
        switch ($read) {
            case TRUE;
                return $this->class
                                ->db
                                ->update("notification", array("read" => 1), array("id_notification" => $id)
                );
            case FALSE:
                return $this->class
                                ->db
                                ->update("notification", array("read" => 0), array("id_notification" => $id)
                );
        }
    }

    public function AddNotification($table, $object, $description, $redirect, $icon, $alert, $roles = NULL, $user = NULL) {
    ...
    public function UpdateNotification($id_notification, $status, $description = NULL, $redirect = NULL, $roles = NULL, $user = NULL) {
        ...
        return $this->class->db->update("notification", $rebase, array("id_notification" => $id_notification));
    }
